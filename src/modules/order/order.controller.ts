...
import { getOrderSchema, deleteOrderSchema, addOrderSchema, updateOrderSchema } from "./order.validation";

const router = Router();
const orderService = new OrderService();

router.post("/add-order", auth,validation(addOrderSchema), orderService.addOrder);
router.get("/get-orders", auth, validation(getOrderSchema), orderService.getOrders);
